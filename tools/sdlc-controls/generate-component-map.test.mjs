...
const graph = {
  nodes: {
    'payments-api': {
      ...
      type: 'app',
      data: {
        root: 'packages/payments-api',
        sourceRoot: 'packages/payments-api/src',
    ...
    'untagged-lib': {
      name: 'untagged-lib',
      type: 'lib',
      data: { root: 'packages/untagged-lib', tags: [] },
    },
    'consumer-a': {
      name: 'consumer-a',
      type: 'lib',
      data: { root: 'packages/consumer-a', tags: ['criticality:low'] },
    },
    'consumer-b': {
      name: 'consumer-b',
      type: 'lib',
      data: { root: 'packages/consumer-b', tags: ['criticality:low'] },
    },
  },
  dependencies: {
    'payments-api': [
    ...
    'consumer-a': [
      { source: 'consumer-a', target: 'shared-utils', type: 'static' },
    ],
    'consumer-b': [
      { source: 'consumer-b', target: 'shared-utils', type: 'static' },
    ],
    'marketing-site': [
      { source: 'marketing-site', target: 'untagged-lib', type: 'static' },
    ],
    'shared-utils': [],
    'untagged-lib': [],
  },
...
test('fan-in counts distinct dependents, ignoring npm targets and duplicate edges', () => {
  ...
  assert.equal(
    counts.get('shared-utils'),
    3,
    'payments-api, consumer-a, consumer-b — counted once each'
  );
...
test('a library at or above the fan-in threshold is shared; a leaf is not', () => {
  ...
  assert.equal(
    SHARED_FANIN_THRESHOLD,
    3,
    'the documented threshold this fixture is built around'
  );
  assert.equal(byId(map, 'shared-utils').shared, true);
  assert.equal(byId(map, 'marketing-site').shared, false);
  assert.equal(
    byId(map, 'untagged-lib').shared,
    false,
    'fan-in 1 is below the threshold'
  );
});
...
test('criticality comes from the tag, never from fan-in', () => {
  const map = buildComponentMap(graph);
  assert.equal(
    byId(map, 'payments-api').criticality,
    'critical',
    'from criticality:critical'
  );
...
test('an untagged project falls back to the conservative default', () => {
  ...
  assert.equal(
    byId(buildComponentMap(graph), 'untagged-lib').criticality,
    'high'
  );
});
...
test('path globs cover the project root', () => {
  const map = buildComponentMap(graph);
  assert.deepEqual(byId(map, 'payments-api').match, [
...
test('a sourceRoot outside the project root gets its own glob', () => {
  assert.deepEqual(
    matchesFor('p', {
      root: 'packages/p',
      sourceRoot: 'generated/p/src',
    }).sort(),
    ['generated/p/src/**', 'packages/p/**']
  );
});

test('a bad criticality tag fails rather than being tiered around', () => {
  assert.throws(
    () => criticalityOf('p', ['criticality:critcal']),
    /not one of low\|medium\|high\|critical/
  );
  assert.throws(
    () => criticalityOf('p', ['criticality:low', 'criticality:high']),
    /several criticality tags/
  );
});

test('a root project is rejected: its glob would claim every path', () => {
  assert.throws(
    () => matchesFor('rooty', { root: '.' }),
    /cannot be mapped to a path glob/
  );
});
...
test('an Nx project colliding with a workspace component is rejected', () => {
  const colliding = {
    nodes: {
      docs: { name: 'docs', data: { root: 'packages/docs', tags: [] } },
    },
    dependencies: {},
  };
...
test('output is deterministic and sorted regardless of graph key order', () => {
  const reversed = {
    ...
    dependencies: Object.fromEntries(
      Object.entries(graph.dependencies).reverse()
    ),
  };
  assert.equal(
    toYaml(buildComponentMap(graph)),
    toYaml(buildComponentMap(reversed))
  );
...
test('an empty graph still yields a valid map: the workspace components alone', () => {
  ...
  assert.equal(
    byId(map, 'sdlc-controls-gate').criticality,
    'critical',
    'the generator governs itself'
  );
});
